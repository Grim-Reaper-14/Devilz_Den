...
constexpr ULONGLONG WorldEnvironmentTickIntervalMs = 100;
...
std::array<std::byte, GTA_Run_Script_Threads_Bridge::AbsoluteJumpSize> AbsoluteJump(std::uintptr_t destination) noexcept
{
...
bool WriteExecutableBytes(std::uintptr_t address, const std::byte* bytes, std::size_t size) noexcept
{
...
GTA_Run_Script_Threads_Bridge::~GTA_Run_Script_Threads_Bridge() { Uninstall(); }
    ...
    Backend::LoggerService& logger) noexcept
{
    ...
    if (runScriptThreadsAddress == 0 || scriptThreadsStorageAddress == 0 || expectedThreadDispatchAddress == 0 ||
        !natives.Ready() || s_active != nullptr) return false;
    if (!IsReadableAddress(runScriptThreadsAddress, PatchSize)) return false;
    ...
    if (current != VerifiedPrologue) return false;

    constexpr std::size_t trampolineSize = PatchSize + AbsoluteJumpSize;
    auto* trampoline = ::VirtualAlloc(nullptr, trampolineSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
    if (!trampoline) return false;
    std::memcpy(trampoline, current.data(), current.size());
    const auto returnJump = AbsoluteJump(runScriptThreadsAddress + PatchSize);
    std::memcpy(static_cast<std::byte*>(trampoline) + PatchSize, returnJump.data(), returnJump.size());
    ...
    logger.Log(Backend::LogLevel::Info,
...
void GTA_Run_Script_Threads_Bridge::Uninstall() noexcept
{
    ...
    if (m_targetAddress != 0) WriteExecutableBytes(m_targetAddress, m_originalBytes.data(), m_originalBytes.size());
    while (m_activeCalls.load() != 0) ::Sleep(0);
    ...
    m_nextWorldEnvironmentTickMs = 0;
    m_slowExtensionCursor = 0;
    m_snapshotExtensionCursor = 0;
    m_fastRunApplied = false;
    m_fastSwimApplied = false;
...
bool GTA_Run_Script_Threads_Bridge::HookThunk(int opsToExecute)
{
    ...
    if (!bridge || !bridge->m_original) return false;
...
bool GTA_Run_Script_Threads_Bridge::OnRunScriptThreads(int opsToExecute) noexcept
{
    ...
    if (m_installed.load(std::memory_order_relaxed) && m_natives) RunGameThreadFeatureTick();
    return result;
}
...
{
    bool expected = false;
    if (!m_smokeAttempting.compare_exchange_strong(expected, true)) return;
    if (!m_natives || !m_logger || !m_natives->Ready()) { m_smokeAttempting.store(false); return; }
    void* scriptThread = FindValidatedScriptThread();
    if (!scriptThread) { m_smokeAttempting.store(false); return; }
    const auto tlsArray = static_cast<std::uintptr_t>(__readgsqword(0x58));
    if (!IsReadableAddress(tlsArray, sizeof(void*))) { m_smokeAttempting.store(false); return; }
    ...
    if (!tls || !IsReadableAddress(reinterpret_cast<std::uintptr_t>(tls), sizeof(GTA_Tls_Context_View))) {
        m_smokeAttempting.store(false); return;
    }
    ...
    if (timer) {
        ...
        m_logger->Log(Backend::LogLevel::Info,
            "GET_GAME_TIMER invocation succeeded | Result: " + std::to_string(*timer) + " | Execution: RunScriptThreads",
            "GTA5_Enhanced.Natives");
    }
    m_smokeAttempting.store(false);
}
...
{
    if (!m_natives || !m_natives->Ready()) return;
    const ULONGLONG now = ::GetTickCount64();
    auto& gameplayState = GTA_Gameplay_State::Instance();
    const auto personalSaveStatus = VehiclePersonalSaveStatus();
    const bool personalSaveActive = personalSaveStatus == GTA_Vehicle_Personal_Save_Status::Queued ||
    ...
    const bool frameFeatureActive = gameplayState.SuperJump() || gameplayState.FastRun() || gameplayState.FastSwim() ||
    ...
    if (!frameDue && !scheduledDue) return;

    void* scriptThread = FindValidatedScriptThread();
    if (!scriptThread) return;
    const auto tlsArray = static_cast<std::uintptr_t>(__readgsqword(0x58));
    if (!IsReadableAddress(tlsArray, sizeof(void*))) return;
    GTA_Tls_Context_View* tls = nullptr;
    std::memcpy(&tls, reinterpret_cast<const void*>(tlsArray), sizeof(tls));
    if (!tls || !IsReadableAddress(reinterpret_cast<std::uintptr_t>(tls), sizeof(GTA_Tls_Context_View))) return;
    void* previousThread = tls->currentScriptThread;
    const bool previousActive = tls->scriptThreadActive;
    tls->currentScriptThread = scriptThread;
    tls->scriptThreadActive = true;
    if (frameDue) {
        ...
        if (personalSaveActive) TickVehiclePersonalSave(*m_natives);
        if (pedControlActive) TickPedControl(*m_natives);
        m_gameplay.TickFrameSensitive();
        if (gameplayState.ExplosiveBullets()) TickExplosiveAmmoExtension(*m_natives, scriptThread);
        TickFrameSensitiveMovement();
    }
    if (scheduledDue) RunLegacyGameplayTick(now);
    tls->scriptThreadActive = previousActive;
    tls->currentScriptThread = previousThread;
}
...
void GTA_Run_Script_Threads_Bridge::TickFrameSensitiveMovement() noexcept
{
    ...
    if (!fastRun && !fastSwim && !m_fastRunApplied && !m_fastSwimApplied) return;
    ...
    if (fastRun) {
        ...
        if (ped && *ped != 0) (void)m_natives->InvokeDirectHash<void>(SetPedMoveRateOverrideHash, *ped, FastRunMoveRateOverride);
    }
...
void GTA_Run_Script_Threads_Bridge::RunLegacyGameplayTick(std::uint64_t now) noexcept
{
    ...
    if (HasPendingScriptFunctionInvocation()) { (void)TickScriptFunctionInvoker(); return; }
    if (WorldEnvironmentHasWork() && now >= m_nextWorldEnvironmentTickMs) {
        m_nextWorldEnvironmentTickMs = now + WorldEnvironmentTickIntervalMs;
    ...
    if (now >= m_nextSelfUtilityTickMs) { m_nextSelfUtilityTickMs = now + SelfUtilityTickIntervalMs; TickSelfUtilityExtension(*m_natives); return; }
    if (now >= m_nextSelfCacheTickMs) { m_nextSelfCacheTickMs = now + SelfCacheTickIntervalMs; GTA_Self_Cache::Instance().Update(*m_natives); return; }
    if (now >= m_nextOnlineExtensionTickMs) { m_nextOnlineExtensionTickMs = now + OnlineExtensionTickIntervalMs; TickSelfOnlineExtension(*m_natives); return; }
    if (now >= m_nextSlowExtensionTickMs) {
        m_nextSlowExtensionTickMs = now + SlowExtensionSliceIntervalMs;
        switch (m_slowExtensionCursor) {
        case 0: TickNetworkSessionExtension(); break;
        case 1: TickRandomEventsExtension(*m_natives); break;
        default: TickBunkerExtension(); break;
        }
        m_slowExtensionCursor = static_cast<std::uint8_t>((m_slowExtensionCursor + 1U) % 3U);
        return;
    }
    if (now >= m_nextVehicleExtensionTickMs) { m_nextVehicleExtensionTickMs = now + VehicleExtensionTickIntervalMs; TickVehicleEditorExtensions(*m_natives); return; }
    if (now >= m_nextSnapshotExtensionTickMs) {
        m_nextSnapshotExtensionTickMs = now + SnapshotExtensionSliceIntervalMs;
        if (m_snapshotExtensionCursor == 0) TickBusinessExtension(*m_natives); else TickCasinoExtension(*m_natives);
...
void* GTA_Run_Script_Threads_Bridge::FindValidatedScriptThread() const noexcept
{
    if (void* cached = m_cachedScriptThread.load(std::memory_order_relaxed)) {
        if (IsReadableAddress(reinterpret_cast<std::uintptr_t>(cached), sizeof(std::uintptr_t))) {
            ...
            if (dispatch == m_expectedThreadDispatchAddress) return cached;
        }
        m_cachedScriptThread.store(nullptr, std::memory_order_relaxed);
    }
    if (!IsReadableAddress(m_scriptThreadsStorageAddress, sizeof(std::uintptr_t))) return nullptr;
    std::uintptr_t slotsAddress = 0;
    std::memcpy(&slotsAddress, reinterpret_cast<const void*>(m_scriptThreadsStorageAddress), sizeof(slotsAddress));
    constexpr std::size_t slotCount = 8;
    if (!IsReadableAddress(slotsAddress, slotCount * sizeof(std::uintptr_t))) return nullptr;
    for (std::size_t i = 0; i < slotCount; ++i) {
        std::uintptr_t candidate = 0;
        std::memcpy(&candidate, reinterpret_cast<const void*>(slotsAddress + i * sizeof(std::uintptr_t)), sizeof(candidate));
        if (!IsReadableAddress(candidate, sizeof(std::uintptr_t))) continue;
        std::uintptr_t dispatch = 0;
